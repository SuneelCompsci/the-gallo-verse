...
while running:
    ...
    #sun
    if suneel_increment < 640:
        suneel_increment += 5
    else:
        suneel_increment = 0
    pygame.draw.circle(screen, (255, 255, 0),
                       (x + x_suneel + 30 + suneel_increment, y + 120), 30)

    #builing drawings
    buildings = 6 #number of buildings you want
    for i in range(buildings):
        building_x_suneel = i * 100
        building_width_suneel = 80
        building_width_suneel = 80
        building_height_suneel = 200
        pygame.draw.rect(screen, (70, 70, 70), (building_x_suneel + x,
                                                y + height - building_height_suneel,
                                                building_width_suneel, building_height_suneel))

        #window drawing
        window_width_suneel = 20
        window_height_suneel = 20
        window_spacing_suneel = 30
        for window_y in range(y + height - building_height_suneel + window_spacing_suneel,
                              y + height - window_height_suneel, window_spacing_suneel):
            for window_x in range(building_x_suneel + x + window_spacing_suneel,
                                  building_x_suneel + x + building_width_suneel - window_width_suneel,
                                  window_spacing_suneel):
                pygame.draw.rect(screen, (255, 255, 255),
                                 (window_x, window_y, window_width_suneel, window_height_suneel))
    # -----------------------
